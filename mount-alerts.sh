#!/bin/bash
	...
	while [ ${a} -gt  0 ]
		do
			...
			$( echo "Changes detected on mount point ${m_p} it is newly added mount point with current usage of ${used}B out of allocated ${total}B" | sendmail  $(whoami) "Alerts RE: Mount Points" )		
